...
int packet_queue_num(packet_queue_t *q) { return q->nb_packets; }
...
int packet_queue_push(packet_queue_t *q, AVPacket *pkt) {
  ...
  if ((pkt != NULL) && (pkt->data != NULL) &&
      (av_packet_make_refcounted(pkt) < 0)) {
...
int audio_decode_frame(AVCodecContext *p_codec_ctx, AVPacket *p_packet,
  ...
  while (1) {
    ...
    if (ret != 0) {
      if (ret == AVERROR_EOF) {
        printf(
            "audio avcodec_receive_frame(): the decoder has been fully "
            "flushed\n");
        res = 0;
        goto exit;
      } else if (ret == AVERROR(EAGAIN)) {
        // printf("audio avcodec_receive_frame(): output is not available in
        // this state - "
        //       "user must try to send new input\n");
        need_new = true;
      } else if (ret == AVERROR(EINVAL)) {
        printf(
            "audio avcodec_receive_frame(): codec not opened, or it is an "
            "encoder\n");
        res = -1;
        goto exit;
      } else {
      ...
      // 此处表示：如果frame中的音频参数 == s_audio_param_src ==
      // s_audio_param_tgt，那音频重采样的过程就免了(因此时s_audio_swr_ctx是NULL)
      ...
      if (p_frame->format != s_audio_param_src.fmt ||
        ...
        s_audio_swr_ctx = swr_alloc_set_opts(
            NULL, s_audio_param_tgt.channel_layout, s_audio_param_tgt.fmt,
            s_audio_param_tgt.freq, p_frame->channel_layout,
            (AVSampleFormat)p_frame->format,
            p_frame->sample_rate, 0, NULL);
        if (s_audio_swr_ctx == NULL || swr_init(s_audio_swr_ctx) < 0) {
          printf(
              ...
              p_frame->sample_rate,
              av_get_sample_fmt_name((AVSampleFormat)p_frame->format),
              p_frame->channels, s_audio_param_tgt.freq,
              av_get_sample_fmt_name(s_audio_param_tgt.fmt),
              s_audio_param_tgt.channels);
          swr_free(&s_audio_swr_ctx);
          return -1;
        }
      ...
      if (s_audio_swr_ctx != NULL)  // 重采样
      {
        ...
        int out_count = (int64_t)p_frame->nb_samples * s_audio_param_tgt.freq /
                            p_frame->sample_rate +
                        256;
        // 重采样输出参数：输出音频缓冲区尺寸(以字节为单位)
        int out_size =
            av_samples_get_buffer_size(NULL, s_audio_param_tgt.channels,
                                       out_count, s_audio_param_tgt.fmt, 0);
        if (out_size < 0) {
        ...
        nb_samples = swr_convert(s_audio_swr_ctx, out, out_count, in,
                                 p_frame->nb_samples);
        if (nb_samples < 0) {
        ...
        if (nb_samples == out_count) {
          ...
          if (swr_init(s_audio_swr_ctx) < 0) swr_free(&s_audio_swr_ctx);
        }

        // 重采样返回的一帧音频数据大小(以字节为单位)
        p_cp_buf = s_resample_buf;
        cp_len = nb_samples * s_audio_param_tgt.channels *
                 av_get_bytes_per_sample(s_audio_param_tgt.fmt);
      } else  // 不重采样
      {
        // 根据相应音频参数，获得所需缓冲区大小
        frm_size = av_samples_get_buffer_size(NULL, p_codec_ctx->channels,
                                              p_frame->nb_samples,
                                              p_codec_ctx->sample_fmt, 1);
...
void sdl_audio_callback(void *userdata, uint8_t *stream, int len) {
  ...
  static uint8_t
      s_audio_buf[(MAX_AUDIO_FRAME_SIZE * 3) / 2];  // 1.5倍声音帧的大小
...
// 通过interval参数传入当前的timer
// interval，返回下一次timer的interval，返回0表示取消定时器
...
int video_thread(void *arg) {
  ...
  while (1) {
    ...
    if (!flush) {
      ...
      //      第一个 flush packet 会返回成功，后续的 flush packet
      //      会返回AVERROR_EOF
      ret = avcodec_send_packet(p_codec_ctx, p_packet);
      if (ret != 0) {
        if (ret == AVERROR_EOF) {
          ...
          printf(
              "video avcodec_send_packet(): input is not accepted in the "
              "current state\n");
        } else if (ret == AVERROR(EINVAL)) {
          printf(
              "video avcodec_send_packet(): codec not opened, it is an "
              "encoder, or requires flush\n");
        } else if (ret == AVERROR(ENOMEM)) {
          printf(
              "video avcodec_send_packet(): failed to add packet to internal "
              "queue, or similar\n");
        } else {
    ...
    if (ret != 0) {
      if (ret == AVERROR_EOF) {
        ...
        printf(
            "video avcodec_receive_frame(): output is not available in this "
            "state - "
            "user must try to send new input\n");
        continue;
      } else if (ret == AVERROR(EINVAL)) {
        printf(
            "video avcodec_receive_frame(): codec not opened, or it is an "
            "encoder\n");
      } else {
...
int open_audio_stream(AVFormatContext *p_fmt_ctx, AVCodecContext *p_codec_ctx,
                      int steam_idx) {
  ...
  wanted_spec.samples =
      SDL_AUDIO_BUFFER_SIZE;  // SDL声音缓冲区尺寸，单位是单声道采样点尺寸x通道数
  ...
  // 然后送再写入SDL音频缓冲区
  s_audio_param_tgt.fmt = AV_SAMPLE_FMT_FLT;
  ...
  s_audio_param_tgt.frame_size = av_samples_get_buffer_size(
      NULL, actual_spec.channels, 1, s_audio_param_tgt.fmt, 1);
  s_audio_param_tgt.bytes_per_sec = av_samples_get_buffer_size(
      NULL, actual_spec.channels, actual_spec.freq, s_audio_param_tgt.fmt, 1);
  if (s_audio_param_tgt.bytes_per_sec <= 0 ||
      s_audio_param_tgt.frame_size <= 0) {
    printf("av_samples_get_buffer_size failed\n");
    return -1;
  }
...
int open_video_stream(AVFormatContext *p_fmt_ctx, AVCodecContext *p_codec_ctx,
                      int steam_idx) {
  AVCodecParameters *p_codec_par = NULL;
  AVCodec *p_codec = NULL;
  int ret=0;
...
int class4(const char *filename) {
  ...
  while (1) {
    ...
    if (ret < 0) {
      if ((ret == AVERROR_EOF) || avio_feof(p_fmt_ctx->pb)) {
        ...
        // 输入文件已读完，发送NULL
        // packet以冲洗(flush)解码器，否则解码器中缓存的帧取不出来
        if (v_idx != -1) {
