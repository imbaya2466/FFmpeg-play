    ...
    if (ret != 0) {
      ...
      if (s_audio_swr_ctx != NULL)  // 重采样
      {
        ...
        if (s_resample_buf == NULL) {
          av_fast_malloc(&s_resample_buf, &s_resample_buf_len,
                         out_size);
        }
        if (s_resample_buf == NULL) {
...
int class3(const char* filename) {
  ...
  if (SDL_OpenAudio(&wanted_spec, &actual_spec) < 0) { //都是SDL的类型
  ...
  // 然后送再写入SDL音频缓冲区
  s_audio_param_tgt.fmt = AV_SAMPLE_FMT_FLT;//FFmpeg的类型这里必须与actual_spec sdl中的format对应，但不是同一值
